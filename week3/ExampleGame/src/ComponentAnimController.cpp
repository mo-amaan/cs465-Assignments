...
// Returns:   void
//
// Register delegates with CharacterController.
// NOTE: only call this after being added to a game object that has a
// character controller attached.
//------------------------------------------------------------------------------
void ComponentAnimController::InitDelegates()
{
	Common::GameObject* pGameObject = this->GetGameObject();
	ComponentBase* pComponent = pGameObject->GetComponent("GOC_CharacterController");
	if (pComponent)
	{
		ComponentCharacterController* pCharController = static_cast<ComponentCharacterController*>(pComponent);
		pCharController->RegisterWalkingDelegate(std::bind(&ComponentAnimController::StartWalkingDelegate, this, std::placeholders::_1));
		pCharController->RegisterStopWalkingDelegate(std::bind(&ComponentAnimController::StopWalkingDelegate, this, std::placeholders::_1));
	}
}

//------------------------------------------------------------------------------
// Method:    StartWalkingDelegate
// Parameter: int p_iTestParam
// Returns:   void
//
// Called when the character controller determines the player starts walking.
//------------------------------------------------------------------------------
void ComponentAnimController::StartWalkingDelegate(int p_iTestParam)
{
	this->SetAnim("walking");
}

//------------------------------------------------------------------------------
// Method:    StopWalkingDelegate
// Parameter: int p_iTestParam
// Returns:   void
//
// Called when the character controller determines the player stops walking.
//------------------------------------------------------------------------------
void ComponentAnimController::StopWalkingDelegate(int p_iTestParam)
{
	this->SetAnim("idle");
}
